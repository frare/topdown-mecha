...
public class Projectile : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            if (hit.collider.gameObject.layer == EnemyTank.shieldLayer) hit.collider.attachedRigidbody.GetComponent<EnemyTank>()?.ShieldDamaged();
            else if (hit.collider.gameObject.layer == Enemy.layer) hit.collider.attachedRigidbody.GetComponent<Enemy>()?.TakeDamage(damage);
